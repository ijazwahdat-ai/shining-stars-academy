...
export default function Navigation({ currentPage, setCurrentPage }: NavigationProps) {
  ...
  return (
    <nav className="bg-gray-900/80 border-b border-gray-800 sticky top-0 z-50 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          >
            ...
            <span className="text-xl font-bold text-white hidden sm:block">{t.academyName}</span>
          </button>
      ...
      {/* Mobile Menus */}
      {isLangMenuOpen && (
        <div className="md:hidden bg-gray-800 border-t border-gray-700 px-4 py-2 flex justify-around">
          {languages.map((lang) => (
            <button
              key={lang.code}
              onClick={() => {
              ...
              className={`px-3 py-1 rounded-full text-sm font-medium ${
                language === lang.code ? 'bg-blue-600 text-white' : 'text-gray-300 bg-gray-700'
              }`}
            >
              {lang.label}
            </button>
